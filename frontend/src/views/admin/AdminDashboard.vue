<template>
  <div class="dashboard-container">
    ...
    <!-- 指标卡片 -->
    <el-row :gutter="16" v-if="reports.length > 0">
    ...
    <!-- 第一行图表：成绩分布 + 错误类型 -->
    <el-row :gutter="16" style="margin-top:16px" v-if="reports.length > 0">
      <el-col :span="12">
        <el-card header="📊 成绩分布">
        ...
        <el-card>
          <template #header>
            ...
            <el-select v-model="selectedStudent" @change="renderRadar" style="width:220px;margin-left:12px" filterable clearable placeholder="搜索学生">
              <el-option v-for="r in reports" :key="r.student_name" :label="r.student_name" :value="r.student_name" />
            </el-select>
          </template>
    ...
    <el-empty v-if="reports.length === 0" description="暂无批改数据，请先完成批改" />
  </div>
</template>
...
const statCards = computed(() => {
  const scores = reports.value.map(r => r.overall_score).filter(s => s !== undefined)
  if (scores.length === 0) return []
  return [
    { icon: '👥', label: '总人数', value: reports.value.length, color: '#409eff' },
...
const warningList = computed(() => {
  return reports.value
    .filter(r => r.overall_score < 60)
    .sort((a, b) => a.overall_score - b.overall_score)
})

const warningColor = (score) => {
...
const renderAllCharts = () => {
  ...
  // 成绩分布
  if (histRef.value) {
    const existing = echarts.getInstanceByDom(histRef.value)
  ...
  // 错误类型
  if (errorRef.value) {
    const existing = echarts.getInstanceByDom(errorRef.value)
    if (existing) existing.dispose()
  ...
  // 薄弱知识点
  if (kpRef.value) {
    const existing = echarts.getInstanceByDom(kpRef.value)
    if (existing) existing.dispose()
    ...
    chart.setOption({
      series: [{
        type: 'pie',
        data: Object.entries(counts).map(([n, v]) => ({ name: n, value: v })),
        radius: ['45%', '75%'],
        label: { formatter: '{b}\n{d}%' }
      }]
    })
  ...
  // 知识点热力图
  if (heatRef.value) {
    const existing = echarts.getInstanceByDom(heatRef.value)
    if (existing) existing.dispose()
    const chart = echarts.init(heatRef.value)
    const students = reports.value.map(r => r.student_name)
    const allTypes = [...new Set(reports.value.flatMap(r => (r.deductions || []).map(d => d.type)))]
    const data = []
    reports.value.forEach((r, i) => {
      r.deductions?.forEach(d => {
        const j = allTypes.indexOf(d.type)
        if (j >= 0) data.push([j, i, d.points_deducted])
      })
    })
    chart.setOption({
...
const renderRadar = () => {
  ...
  const r = reports.value.find(r => r.student_name === selectedStudent.value)
  if (!r) return
...
onMounted(() => {
  nextTick(() => {
    if (reports.value.length > 0) {
      selectedStudent.value = reports.value[0].student_name
    }
    renderAllCharts()
    renderRadar()
  })
...
watch(reports, () => {
  nextTick(() => {
    if (reports.value.length > 0 && !selectedStudent.value) {
      selectedStudent.value = reports.value[0].student_name
    }
    renderAllCharts()
    renderRadar()
